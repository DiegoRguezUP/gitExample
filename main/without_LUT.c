// oneshot_read_main_calc.c
/*
 * Canal 5 -> % Humedad con cálculo directo (expf)
 * Mide tiempo del cálculo y tiempo entre logs
 * RH(%) = 3.0504 * exp(0.0012 * mV)
 */
#include <stdio.h>
#include <math.h>
...
{
    adc_cali_handle_t handle = NULL; esp_err_t ret = ESP_FAIL; bool ok = false;
...
void app_main(void)
{
    ...
    while (1) {
        ...
        // --- medir solo el cálculo mV->%HR ---
        int64_t t0 = esp_timer_get_time();
        float rh = have_cal ? humidity_from_mv((float)mv) : 0.0f;
        ...
        // Usar PRId32 para int32_t
        printf("RH: %.1f %% | calc: %" PRId32 " us | interval: %" PRId32 " ms\n",
